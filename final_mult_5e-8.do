*Multiplicative @ p=5e-8 - main analyses + sex differences + adjusting for covariate interactions
...
foreach l of local levels {
	...
	*Store the results in the excel file
	putexcel A`x'="Smoking combined P=5x10-8 (SD)" B`x'="Smoking" C`x'="Interaction" D`x'="Multiplicative" E`x'=`beta_int' F`x'=`lci_int' G`x'=`uci_int' I`x'=`p_value'

	*Next, the sex stratified interaction coefficients 
	levelsof sex_level, local(levels)
	local x = 7
...
foreach l of local levels {
	...
	local uci_s = (r(ci_upp))
	local p_val_s = r(p_z)	
	putexcel A`x'="Smoking combined P=5x10-8 (SD)" B`x'="Smoking" C`x'="`l' sex" D`x'="Multiplicative" E`x'=`beta_s' F`x'=`lci_s' G`x'=`uci_s' I`x'=`p_val_s'
	...
	*Store the results in the excel file
	putexcel A`x'="Maltreatment score" B`x'="Smoking" C`x'="Interaction" D`x'="Multiplicative" E`x'=`mal_beta' F`x'=`mal_lci' G`x'=`mal_uci' H`x'=`mal_p'
	...
	*Then the maltreatment coefficient but sex stratified 
	...
	metan mal_beta mal_lci mal_uci if sex_level==`l' & mal_smoking_interact==1, lcols(sample) effect(OR) null(1) xlabel(-0.0001, -0.001, -0.01, 0.0) astext(50) dp(6) nograph
	...
	local uci_s = (r(ci_upp))
	local p_val_s = r(p_z)	
	putexcel A`x'="Maltreatment score" B`x'="Smoking" C`x'="`l' sex" D`x'="Multiplicative" E`x'=`beta_s' F`x'=`lci_s' G`x'=`uci_s' H`x'=`p_val_s'
	...
	*Store the results in the excel file
	putexcel  C`x'=" 3 way Interaction" D`x'="Multiplicative" E`x'=`b_3' F`x'=`b_3' G`x'=`b_3' I`x'=`p_3'
...
*Create a varaible to identify the sex for the sex-stratified coefficients 
generate sex_level=.
replace sex_level = 0 if sex=="0 sex"
replace sex_level = 1 if sex=="1 sex"
...
foreach l of local levels {
	...
	*Store the results in the excel file
	putexcel A`x'="SBP combined P=5x10-8 (SD)" B`x'="SBP" C`x'="Interaction" D`x'="Multiplicative" E`x'=`beta_int' F`x'=`lci_int' G`x'=`uci_int' I`x'=`p_value'
...
foreach l of local levels {
	...
	local uci_s = (r(ci_upp))
	local p_val_s = r(p_z)	
	putexcel A`x'="SBP combined P=5x10-8 (SD)" B`x'="SBP" C`x'="`l' sex" D`x'="Multiplicative" E`x'=`beta_s' F`x'=`lci_s' G`x'=`uci_s' I`x'=`p_val_s'
	...
	*Store the results in the excel file
	putexcel A`x'="Maltreatment score" B`x'="SBP" C`x'="Interaction" D`x'="Multiplicative" E`x'=`mal_beta' F`x'=`mal_lci' G`x'=`mal_uci' H`x'=`mal_p'
	...
	*Then the maltreatment coefficient but sex stratified 
	...
	metan mal_beta mal_lci mal_uci if sex_level==`l' & mal_sbp_interact==1, lcols(sample) effect(OR) null(1) xlabel(-0.0001, -0.001, -0.01, 0.0) astext(50) dp(6) nograph
	...
	local uci_s = (r(ci_upp))
	local p_val_s = r(p_z)	
	putexcel A`x'="Maltreatment score" B`x'="SBP" C`x'="`l' sex" D`x'="Multiplicative" E`x'=`beta_s' F`x'=`lci_s' G`x'=`uci_s' H`x'=`p_val_s'
